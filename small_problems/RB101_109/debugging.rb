...
#Our countdown to launch isn't behaving as expected. Why? Change the code so that our 
#program successfully counts down from 10 to 1.
...
#Card Deck
#first issue - error
#second issue - sum is too low
#the problem is the deck has all cards removed from all suits
# require "pry"
...
#original answer in debugging.md
...
# todo = ['study']
# done = ['apply sunscreen', 'walk the dog', 'go to the beach', 'coffee with Tom']

# move(2, todo, done)

# p todo # should be: ['coffee with Tom']
# p done # should be: ['apply sunscreen', 'go to the beach', 'study', 'walk the dog']
# #this error means that we are in an infinite recursive loop. This happened because we
...
#I think what is happening isi you are mutating the element you are iterating over, so 
#when you delete dull, you skip over boring 
...
#Password
#fix the error
#after that, it still doesnt run properly, make sure you can log in with the new password
# require 'pry'
...
#this throws an error because password is initialized outside of the method definition. A
#method definition is limited to the scope of its own block. The password needs to be 
#entered as an argument. 

#after that is fixed, you will still get a failure because when you set the passwpord in
#the set_password method, you set the local variable password to new_password, but when 
#the test runs in verify_password, it is referencing the password variable defined on
#line 462.
...
#this way , the password variable is reset because the block of the if statemetn is in the
#same scope as the original password variable so it can change it

#mine works but the better way is actually to return the new password from the set_password
#method, and reset the password like that, 
...
#problem now is it is giving me an unlimited number of attempts, because the recursive call
#sets attempts to 0 everytime
...
# #seems like its wrong when the word is in 2 but not the other, it goes to zero
# #problem is in idf method, it returns zero for all cat examples
# #
...
#cant remove any code
#print array of numeric strings in reverse numerical order
require 'pry'
arr = ["9", "8", "7", "10", "11"]
p (arr.sort do |x, y|
    y.to_i <=> x.to_i
